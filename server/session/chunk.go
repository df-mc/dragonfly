...
var overworldEmptyChunk, netherEmptyChunk, endEmptyChunk emptyChunkData
...
func init() {
	...
	for dim, chunkData := range map[world.Dimension]*emptyChunkData{
		...
		world.End:       &endEmptyChunk,
	} {
		data := chunk.Encode(chunk.New(air, dim.Range()), chunk.NetworkEncoding)
		...
		buf.WriteByte(0)
		*chunkData = emptyChunkData{payload: buf.Bytes(), subChunkCount: uint32(len(data.SubChunks))}
	}
}

// emptyChunk returns the correct data to send an empty chunk for the provided dimension.
func emptyChunk(dimension int32) emptyChunkData {
	if dimension == packet.DimensionNether {
		return netherEmptyChunk
	} else if dimension == packet.DimensionEnd {
		return endEmptyChunk
	}
	return overworldEmptyChunk
}
