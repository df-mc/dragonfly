...
func (EnchantedApple) Consume(_ *world.World, c Consumer) Stack {
	c.Saturate(4, 9.6)
	c.AddEffect(effect.Absorption{}.WithSettings(2*time.Minute, 4, false, false))
	c.AddEffect(effect.Regeneration{}.WithSettings(30*time.Second, 5, false, false))
	c.AddEffect(effect.FireResistance{}.WithSettings(5*time.Minute, 1, false, false))
	c.AddEffect(effect.Resistance{}.WithSettings(5*time.Minute, 1, false, false))
	return Stack{}
}
