...
type BoneMealAffected interface {
	...
	BoneMeal(pos cube.Pos, creative bool, tx *world.Tx) bool
}

// UseOnBlock ...
func (b BoneMeal) UseOnBlock(pos cube.Pos, _ cube.Face, _ mgl64.Vec3, tx *world.Tx, user User, ctx *UseContext) bool {
	creative := false
	if gm, ok := user.(interface{ GameMode() world.GameMode }); ok {
		creative = gm.GameMode().CreativeInventory()
	}
	if bm, ok := tx.Block(pos).(BoneMealAffected); ok && bm.BoneMeal(pos, creative, tx) {
