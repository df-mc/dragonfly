...
type Piston struct {
	...
	Facing cube.Face
	// Sticky is true if the piston is a sticky piston.
	Sticky bool
...
func (p Piston) extend(pos cube.Pos, tx *world.Tx) bool {
	...
	tx.SetBlock(pushPos, PistonArmCollision{Facing: p.Facing, Sticky: p.Sticky}, nil)
	return true
}
...
func (p Piston) isImmovable(b world.Block) bool {
	...
	// TODO: Add TileEntities check once implemented.
	return false
}
...
func (p Piston) EncodeBlock() (string, map[string]any) {
	...
	return name, map[string]any{"facing_direction": int32(p.Facing), "extended_bit": p.Extended}
}
	...
	for _, face := range cube.Faces() {
		for _, sticky := range []bool{false, true} {
			for _, extended := range []bool{false, true} {
				pistons = append(pistons, Piston{Facing: face, Sticky: sticky, Extended: extended})
			}
		}
	}
	return
