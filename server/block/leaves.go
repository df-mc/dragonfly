...
func (l Leaves) BreakInfo() BreakInfo {
	return newBreakInfo(0.2, alwaysHarvestable, func(t item.Tool) bool {
		...
		if sapling, ok := l.Type.Sapling(); ok {
			saplingChances := []float64{0.05, 0.0625, 0.083333333, 0.1}
			if l.Type == JungleLeaves() {
				saplingChances = []float64{0.025, 0.027777778, 0.03125, 0.041666667}
			}
			if rand.Float64() < saplingChances[min(fortune, 3)] {
