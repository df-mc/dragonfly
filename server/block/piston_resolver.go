...
// pistonResolve ...
func pistonResolve(tx *world.Tx, pos cube.Pos, piston Piston, push bool) *pistonResolver {
	r := &pistonResolver{
		tx:  tx,
		pos: pos,
...
func (r *pistonResolver) calculateBlocks(pos cube.Pos, face cube.Face, breakFace cube.Face) bool {
	if pos.Side(breakFace).OutOfBounds(r.tx.Range()) {
	...
	// account for slime.
	if !r.calculateBlocks(pos.Side(breakFace), breakFace, breakFace) {
		return false
	}
