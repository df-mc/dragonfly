...
func (t IronTrapDoor) RedstoneUpdate(pos cube.Pos, tx *world.Tx) {
	...
	if !t.Open {
		...
		tx.SetBlock(pos, t, &world.SetOpts{DisableBlockUpdates: true})
	} else {
		tx.ScheduleBlockUpdate(pos, t, time.Millisecond*50)
	}
}

// ScheduledTick ...
func (t IronTrapDoor) ScheduledTick(pos cube.Pos, tx *world.Tx, _ *rand.Rand) {
