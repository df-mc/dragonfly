...
func (b Beacon) DecodeNBT(data map[string]any) any {
	b.level = int(nbtconv.Read[int32](data, "Levels"))
	if primary, ok := effect.ByID(int(nbtconv.Read[int32](data, "Primary"))); ok {
		b.Primary = primary.(effect.LastingType)
	}
	if secondary, ok := effect.ByID(int(nbtconv.Read[int32](data, "Secondary"))); ok {
