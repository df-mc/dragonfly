...
type Chest struct {
	...
	inventory *world.InventoryHandle
}
...
func (c Chest) Inventory() *world.InventoryHandle {
	return c.inventory
}
...
func (c Chest) UseOnBlock(pos cube.Pos, face cube.Face, _ mgl64.Vec3, tx *world.Tx, user item.User, ctx *item.UseContext) (used bool) {
	...
	c.inventory = tx.SetContainer(inventory.New(27, func(slot int, before, after world.ItemStack) {}), &world.ContainerOpts{
...
func (c Chest) BreakInfo() BreakInfo {
	return newBreakInfo(2.5, alwaysHarvestable, axeEffective, oneOf(c)).withBreakHandler(func(pos cube.Pos, tx *world.Tx, u item.User) {
		...
		container := tx.Container(c.Inventory())
		for _, i := range container.Clear() {
