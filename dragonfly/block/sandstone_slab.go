...
// SandstoneSlab is a half block that allows entities to walk up blocks without jumping.
...
func (s SandstoneSlab) UseOnBlock(pos world.BlockPos, face world.Face, clickPos mgl64.Vec3, w *world.World, user item.User, ctx *item.UseContext) (used bool) {
...
func (s SandstoneSlab) LightDiffusionLevel() uint8 {
	if s.Double {
		return 15
	}
	return 0
...
// AABB ...
func (s SandstoneSlab) AABB(world.BlockPos, *world.World) []physics.AABB {
	if s.Double {
...
// EncodeItem ...
func (s SandstoneSlab) EncodeItem() (id int32, meta int16) {
	if s.Double {
		return 43, 1
	}
	return 44, 1
...
// EncodeBlock ...
func (s SandstoneSlab) EncodeBlock() (name string, properties map[string]interface{}) {
	if s.Double {
		return "minecraft:double_stone_slab", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type": "sandstone"}
	}
	return "minecraft:stone_slab", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type": "sandstone"}
}

// Hash ...
func (s SandstoneSlab) Hash() uint64 {
	return hashSandstoneSlab | (uint64(boolByte(s.Top)) << 32) | (uint64(boolByte(s.Double)) << 33)
}
...
// SideClosed ...
func (s SandstoneSlab) SideClosed(pos, side world.BlockPos, _ *world.World) bool {
	// Only returns true if the side is below the slab and if the slab is not upside down.
	return !s.Top && side[1] == pos[1]-1
}

// allSandstoneSlabs returns all states of sandstone slabs.
func allSandstoneSlabs() (slabs []world.Block) {
	f := func(double bool, upsideDown bool) {
		slabs = append(slabs, SandstoneSlab{Double: double, Top: upsideDown})
	}
	f(false, false)
	f(false, true)
	f(true, false)
