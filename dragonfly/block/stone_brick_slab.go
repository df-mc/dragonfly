...
// StoneBrickSlab is a half block that allows entities to walk up blocks without jumping.
...
// Model ...
func (s StoneBrickSlab) Model() world.BlockModel {
...
func (s StoneBrickSlab) UseOnBlock(pos world.BlockPos, face world.Face, clickPos mgl64.Vec3, w *world.World, user item.User, ctx *item.UseContext) (used bool) {
	clickedBlock := w.Block(pos)
	if clickedSlab, ok := clickedBlock.(StoneBrickSlab); ok && !s.Double {
	...
	if sideSlab, ok := w.Block(pos.Side(face)).(StoneBrickSlab); ok && !replaceableWith(w, pos, s) && !s.Double {
...
// BreakInfo ...
func (s StoneBrickSlab) BreakInfo() BreakInfo {
	return BreakInfo{
		Hardness:    2,
...
func (s StoneBrickSlab) LightDiffusionLevel() uint8 {
	if s.Double {
		return 15
	}
	return 0
...
// AABB ...
func (s StoneBrickSlab) AABB(world.BlockPos, *world.World) []physics.AABB {
	if s.Double {
...
// EncodeItem ...
func (s StoneBrickSlab) EncodeItem() (id int32, meta int16) {
	if s.Double {
		return 43, 5
	}
	return 44, 5
...
// EncodeBlock ...
func (s StoneBrickSlab) EncodeBlock() (name string, properties map[string]interface{}) {
	if s.Double {
		return "minecraft:double_stone_slab", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type": "stone_brick"}
	}
	return "minecraft:stone_slab", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type": "stone_brick"}
}

// Hash ...
func (s StoneBrickSlab) Hash() uint64 {
	return hashStoneBrickSlab | (uint64(boolByte(s.Top)) << 32) | (uint64(boolByte(s.Double)) << 33)
}

// CanDisplace ...
func (s StoneBrickSlab) CanDisplace(b world.Liquid) bool {
	_, ok := b.(Water)
	return !s.Double && ok
}

// SideClosed ...
func (s StoneBrickSlab) SideClosed(pos, side world.BlockPos, _ *world.World) bool {
	// Only returns true if the side is below the slab and if the slab is not upside down.
	return !s.Top && side[1] == pos[1]-1
}

// allStoneBrickSlabs returns all states of stone brick slabs.
func allStoneBrickSlabs() (slabs []world.Block) {
	f := func(double bool, upsideDown bool) {
		slabs = append(slabs, StoneBrickSlab{Double: double, Top: upsideDown})
	}
	f(false, false)
	f(false, true)
	f(true, false)
