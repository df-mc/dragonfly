...
// DioriteSlab is a half block that allows entities to walk up blocks without jumping.
...
// Model ...
func (s DioriteSlab) Model() world.BlockModel {
...
func (s DioriteSlab) UseOnBlock(pos world.BlockPos, face world.Face, clickPos mgl64.Vec3, w *world.World, user item.User, ctx *item.UseContext) (used bool) {
	clickedBlock := w.Block(pos)
	if clickedSlab, ok := clickedBlock.(DioriteSlab); ok && !s.Double {
	...
	if sideSlab, ok := w.Block(pos.Side(face)).(DioriteSlab); ok && !replaceableWith(w, pos, s) && !s.Double {
...
// BreakInfo ...
func (s DioriteSlab) BreakInfo() BreakInfo {
	return BreakInfo{
		Hardness:    1.5,
...
func (s DioriteSlab) LightDiffusionLevel() uint8 {
	if s.Double {
		return 15
	}
	return 0
...
// AABB ...
func (s DioriteSlab) AABB(world.BlockPos, *world.World) []physics.AABB {
	if s.Double {
...
// EncodeItem ...
func (s DioriteSlab) EncodeItem() (id int32, meta int16) {
	if s.Double {
		return -167, 4
	}
	return -162, 4
...
// EncodeBlock ...
func (s DioriteSlab) EncodeBlock() (name string, properties map[string]interface{}) {
	if s.Double {
		return "minecraft:double_stone_slab3", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type_3": "diorite"}
	}
	return "minecraft:stone_slab3", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type_3": "diorite"}
}

// Hash ...
func (s DioriteSlab) Hash() uint64 {
	return hashDioriteSlab | (uint64(boolByte(s.Top)) << 32) | (uint64(boolByte(s.Double)) << 33)
}

// CanDisplace ...
func (s DioriteSlab) CanDisplace(b world.Liquid) bool {
	_, ok := b.(Water)
	return !s.Double && ok
}

// SideClosed ...
func (s DioriteSlab) SideClosed(pos, side world.BlockPos, _ *world.World) bool {
	// Only returns true if the side is below the slab and if the slab is not upside down.
	return !s.Top && side[1] == pos[1]-1
}

// allDioriteSlabs returns all states of diorite slabs.
func allDioriteSlabs() (slabs []world.Block) {
	f := func(double bool, upsideDown bool) {
		slabs = append(slabs, DioriteSlab{Double: double, Top: upsideDown})
	}
	f(false, false)
	f(false, true)
	f(true, false)
