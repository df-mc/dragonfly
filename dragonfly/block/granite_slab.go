...
// GraniteSlab is a half block that allows entities to walk up blocks without jumping.
...
// Model ...
func (s GraniteSlab) Model() world.BlockModel {
...
func (s GraniteSlab) UseOnBlock(pos world.BlockPos, face world.Face, clickPos mgl64.Vec3, w *world.World, user item.User, ctx *item.UseContext) (used bool) {
	clickedBlock := w.Block(pos)
	if clickedSlab, ok := clickedBlock.(GraniteSlab); ok && !s.Double {
	...
	if sideSlab, ok := w.Block(pos.Side(face)).(GraniteSlab); ok && !replaceableWith(w, pos, s) && !s.Double {
...
// BreakInfo ...
func (s GraniteSlab) BreakInfo() BreakInfo {
	return BreakInfo{
		Hardness:    1.5,
...
func (s GraniteSlab) LightDiffusionLevel() uint8 {
	if s.Double {
		return 15
	}
	return 0
...
// AABB ...
func (s GraniteSlab) AABB(world.BlockPos, *world.World) []physics.AABB {
	if s.Double {
...
// EncodeItem ...
func (s GraniteSlab) EncodeItem() (id int32, meta int16) {
	if s.Double {
		return -167, 6
	}
	return -162, 6
...
// EncodeBlock ...
func (s GraniteSlab) EncodeBlock() (name string, properties map[string]interface{}) {
	if s.Double {
		return "minecraft:double_stone_slab3", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type_3": "granite"}
	}
	return "minecraft:stone_slab3", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type_3": "granite"}
}

// Hash ...
func (s GraniteSlab) Hash() uint64 {
	return hashGraniteSlab | (uint64(boolByte(s.Top)) << 32) | (uint64(boolByte(s.Double)) << 33)
}

// CanDisplace ...
func (s GraniteSlab) CanDisplace(b world.Liquid) bool {
	_, ok := b.(Water)
	return !s.Double && ok
}

// SideClosed ...
func (s GraniteSlab) SideClosed(pos, side world.BlockPos, _ *world.World) bool {
	// Only returns true if the side is below the slab and if the slab is not upside down.
	return !s.Top && side[1] == pos[1]-1
}

// allGraniteSlabs returns all states of granite slabs.
func allGraniteSlabs() (slabs []world.Block) {
	f := func(double bool, upsideDown bool) {
		slabs = append(slabs, GraniteSlab{Double: double, Top: upsideDown})
	}
	f(false, false)
	f(false, true)
	f(true, false)
