...
// NetherBrickSlab is a half block that allows entities to walk up blocks without jumping.
...
// Model ...
func (s NetherBrickSlab) Model() world.BlockModel {
...
func (s NetherBrickSlab) UseOnBlock(pos world.BlockPos, face world.Face, clickPos mgl64.Vec3, w *world.World, user item.User, ctx *item.UseContext) (used bool) {
	clickedBlock := w.Block(pos)
	if clickedSlab, ok := clickedBlock.(NetherBrickSlab); ok && !s.Double {
	...
	if sideSlab, ok := w.Block(pos.Side(face)).(NetherBrickSlab); ok && !replaceableWith(w, pos, s) && !s.Double {
...
// BreakInfo ...
func (s NetherBrickSlab) BreakInfo() BreakInfo {
	return BreakInfo{
		Hardness:    2,
...
func (s NetherBrickSlab) LightDiffusionLevel() uint8 {
	if s.Double {
		return 15
	}
	return 0
...
// AABB ...
func (s NetherBrickSlab) AABB(world.BlockPos, *world.World) []physics.AABB {
	if s.Double {
...
// EncodeItem ...
func (s NetherBrickSlab) EncodeItem() (id int32, meta int16) {
	if s.Double {
		return 43, 7
	}
	return 44, 7
...
// EncodeBlock ...
func (s NetherBrickSlab) EncodeBlock() (name string, properties map[string]interface{}) {
	if s.Double {
		return "minecraft:double_stone_slab", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type": "nether_brick"}
	}
	return "minecraft:stone_slab", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type": "nether_brick"}
}

// Hash ...
func (s NetherBrickSlab) Hash() uint64 {
	return hashNetherBrickSlab | (uint64(boolByte(s.Top)) << 32) | (uint64(boolByte(s.Double)) << 33)
}

// CanDisplace ...
func (s NetherBrickSlab) CanDisplace(b world.Liquid) bool {
	_, ok := b.(Water)
	return !s.Double && ok
}

// SideClosed ...
func (s NetherBrickSlab) SideClosed(pos, side world.BlockPos, _ *world.World) bool {
	// Only returns true if the side is below the slab and if the slab is not upside down.
	return !s.Top && side[1] == pos[1]-1
}

// allNetherBrickSlabs returns all states of nether brick slabs.
func allNetherBrickSlabs() (slabs []world.Block) {
	f := func(double bool, upsideDown bool) {
		slabs = append(slabs, NetherBrickSlab{Double: double, Top: upsideDown})
	}
	f(false, false)
	f(false, true)
	f(true, false)
