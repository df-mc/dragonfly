...
// QuartzSlab is a half block that allows entities to walk up blocks without jumping.
...
// Model ...
func (s QuartzSlab) Model() world.BlockModel {
...
func (s QuartzSlab) UseOnBlock(pos world.BlockPos, face world.Face, clickPos mgl64.Vec3, w *world.World, user item.User, ctx *item.UseContext) (used bool) {
	clickedBlock := w.Block(pos)
	if clickedSlab, ok := clickedBlock.(QuartzSlab); ok && !s.Double {
	...
	if sideSlab, ok := w.Block(pos.Side(face)).(QuartzSlab); ok && !replaceableWith(w, pos, s) && !s.Double {
...
// BreakInfo ...
func (s QuartzSlab) BreakInfo() BreakInfo {
	return BreakInfo{
		Hardness:    2,
...
func (s QuartzSlab) LightDiffusionLevel() uint8 {
	if s.Double {
		return 15
	}
	return 0
...
// AABB ...
func (s QuartzSlab) AABB(world.BlockPos, *world.World) []physics.AABB {
	if s.Double {
...
// EncodeItem ...
func (s QuartzSlab) EncodeItem() (id int32, meta int16) {
	if s.Double {
		return 43, 6
	}
	return 44, 6
...
// EncodeBlock ...
func (s QuartzSlab) EncodeBlock() (name string, properties map[string]interface{}) {
	if s.Double {
		return "minecraft:double_stone_slab", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type": "quartz"}
	}
	return "minecraft:stone_slab", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type": "quartz"}
}

// Hash ...
func (s QuartzSlab) Hash() uint64 {
	return hashQuartzSlab | (uint64(boolByte(s.Top)) << 32) | (uint64(boolByte(s.Double)) << 33)
}

// CanDisplace ...
func (s QuartzSlab) CanDisplace(b world.Liquid) bool {
	_, ok := b.(Water)
	return !s.Double && ok
}

// SideClosed ...
func (s QuartzSlab) SideClosed(pos, side world.BlockPos, _ *world.World) bool {
	// Only returns true if the side is below the slab and if the slab is not upside down.
	return !s.Top && side[1] == pos[1]-1
}

// allQuartzSlabs returns all states of quartz slabs.
func allQuartzSlabs() (slabs []world.Block) {
	f := func(double bool, upsideDown bool) {
		slabs = append(slabs, QuartzSlab{Double: double, Top: upsideDown})
	}
	f(false, false)
	f(false, true)
	f(true, false)
