...
// CobblestoneSlab is a half block that allows entities to walk up blocks without jumping.
...
// Model ...
func (s CobblestoneSlab) Model() world.BlockModel {
...
func (s CobblestoneSlab) UseOnBlock(pos world.BlockPos, face world.Face, clickPos mgl64.Vec3, w *world.World, user item.User, ctx *item.UseContext) (used bool) {
	clickedBlock := w.Block(pos)
	if clickedSlab, ok := clickedBlock.(CobblestoneSlab); ok && !s.Double {
	...
	if sideSlab, ok := w.Block(pos.Side(face)).(CobblestoneSlab); ok && !replaceableWith(w, pos, s) && !s.Double {
...
// BreakInfo ...
func (s CobblestoneSlab) BreakInfo() BreakInfo {
	return BreakInfo{
		Hardness:    2,
...
func (s CobblestoneSlab) LightDiffusionLevel() uint8 {
	if s.Double {
		return 15
	}
	return 0
...
// AABB ...
func (s CobblestoneSlab) AABB(world.BlockPos, *world.World) []physics.AABB {
	if s.Double {
...
// EncodeItem ...
func (s CobblestoneSlab) EncodeItem() (id int32, meta int16) {
	if s.Double {
		return 43, 3
	}
	return 44, 3
...
// EncodeBlock ...
func (s CobblestoneSlab) EncodeBlock() (name string, properties map[string]interface{}) {
	if s.Double {
		return "minecraft:double_stone_slab", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type": "cobblestone"}
	}
	return "minecraft:stone_slab", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type": "cobblestone"}
}

// Hash ...
func (s CobblestoneSlab) Hash() uint64 {
	return hashCobblestoneSlab | (uint64(boolByte(s.Top)) << 32) | (uint64(boolByte(s.Double)) << 33)
}

// CanDisplace ...
func (s CobblestoneSlab) CanDisplace(b world.Liquid) bool {
	_, ok := b.(Water)
	return !s.Double && ok
}

// SideClosed ...
func (s CobblestoneSlab) SideClosed(pos, side world.BlockPos, _ *world.World) bool {
	// Only returns true if the side is below the slab and if the slab is not upside down.
	return !s.Top && side[1] == pos[1]-1
}

// allCobblestoneSlabs returns all states of cobblestone slabs.
func allCobblestoneSlabs() (slabs []world.Block) {
	f := func(double bool, upsideDown bool) {
		slabs = append(slabs, CobblestoneSlab{Double: double, Top: upsideDown})
	}
	f(false, false)
	f(false, true)
	f(true, false)
