...
// CutSandstoneSlab is a half block that allows entities to walk up blocks without jumping.
...
// Model ...
func (s CutSandstoneSlab) Model() world.BlockModel {
...
func (s CutSandstoneSlab) UseOnBlock(pos world.BlockPos, face world.Face, clickPos mgl64.Vec3, w *world.World, user item.User, ctx *item.UseContext) (used bool) {
	clickedBlock := w.Block(pos)
	if clickedSlab, ok := clickedBlock.(CutSandstoneSlab); ok && !s.Double {
	...
	if sideSlab, ok := w.Block(pos.Side(face)).(CutSandstoneSlab); ok && !replaceableWith(w, pos, s) && !s.Double {
...
// BreakInfo ...
func (s CutSandstoneSlab) BreakInfo() BreakInfo {
	return BreakInfo{
		Hardness:    2,
...
func (s CutSandstoneSlab) LightDiffusionLevel() uint8 {
	if s.Double {
		return 15
	}
	return 0
...
// AABB ...
func (s CutSandstoneSlab) AABB(world.BlockPos, *world.World) []physics.AABB {
	if s.Double {
...
// EncodeItem ...
func (s CutSandstoneSlab) EncodeItem() (id int32, meta int16) {
	if s.Double {
		return -168, 3
	}
	return -166, 3
...
// EncodeBlock ...
func (s CutSandstoneSlab) EncodeBlock() (name string, properties map[string]interface{}) {
	if s.Double {
		return "minecraft:double_stone_slab4", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type_4": "cut_sandstone"}
	}
	return "minecraft:stone_slab4", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type_4": "cut_sandstone"}
}

// Hash ...
func (s CutSandstoneSlab) Hash() uint64 {
	return hashCutSandstoneSlab | (uint64(boolByte(s.Top)) << 32) | (uint64(boolByte(s.Double)) << 33)
}

// CanDisplace ...
func (s CutSandstoneSlab) CanDisplace(b world.Liquid) bool {
	_, ok := b.(Water)
	return !s.Double && ok
}

// SideClosed ...
func (s CutSandstoneSlab) SideClosed(pos, side world.BlockPos, _ *world.World) bool {
	// Only returns true if the side is below the slab and if the slab is not upside down.
	return !s.Top && side[1] == pos[1]-1
}

// allCutSandstoneSlabs returns all states of cut sandstone slabs.
func allCutSandstoneSlabs() (slabs []world.Block) {
	f := func(double bool, upsideDown bool) {
		slabs = append(slabs, CutSandstoneSlab{Double: double, Top: upsideDown})
	}
	f(false, false)
	f(false, true)
	f(true, false)
