...
// SmoothStoneSlab is a half block that allows entities to walk up blocks without jumping.
...
// Model ...
func (s SmoothStoneSlab) Model() world.BlockModel {
...
func (s SmoothStoneSlab) UseOnBlock(pos world.BlockPos, face world.Face, clickPos mgl64.Vec3, w *world.World, user item.User, ctx *item.UseContext) (used bool) {
	clickedBlock := w.Block(pos)
	if clickedSlab, ok := clickedBlock.(SmoothStoneSlab); ok && !s.Double {
	...
	if sideSlab, ok := w.Block(pos.Side(face)).(SmoothStoneSlab); ok && !replaceableWith(w, pos, s) && !s.Double {
...
// BreakInfo ...
func (s SmoothStoneSlab) BreakInfo() BreakInfo {
	return BreakInfo{
		Hardness:    2,
...
func (s SmoothStoneSlab) LightDiffusionLevel() uint8 {
	if s.Double {
		return 15
	}
	return 0
...
// AABB ...
func (s SmoothStoneSlab) AABB(world.BlockPos, *world.World) []physics.AABB {
	if s.Double {
...
// EncodeItem ...
func (s SmoothStoneSlab) EncodeItem() (id int32, meta int16) {
	if s.Double {
		return 43, 0
	}
	return 44, 0
...
// EncodeBlock ...
func (s SmoothStoneSlab) EncodeBlock() (name string, properties map[string]interface{}) {
	if s.Double {
		return "minecraft:double_stone_slab", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type": "smooth_stone"}
	}
	return "minecraft:stone_slab", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type": "smooth_stone"}
}

// Hash ...
func (s SmoothStoneSlab) Hash() uint64 {
	return hashSmoothStoneSlab | (uint64(boolByte(s.Top)) << 32) | (uint64(boolByte(s.Double)) << 33)
}

// CanDisplace ...
func (s SmoothStoneSlab) CanDisplace(b world.Liquid) bool {
	_, ok := b.(Water)
	return !s.Double && ok
}

// SideClosed ...
func (s SmoothStoneSlab) SideClosed(pos, side world.BlockPos, _ *world.World) bool {
	// Only returns true if the side is below the slab and if the slab is not upside down.
	return !s.Top && side[1] == pos[1]-1
}

// allSmoothStoneSlabs returns all states of smooth stone slabs.
func allSmoothStoneSlabs() (slabs []world.Block) {
	f := func(double bool, upsideDown bool) {
		slabs = append(slabs, SmoothStoneSlab{Double: double, Top: upsideDown})
	}
	f(false, false)
	f(false, true)
	f(true, false)
