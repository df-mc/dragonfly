...
// SmoothSandstoneSlab is a half block that allows entities to walk up blocks without jumping.
...
// Model ...
func (s SmoothSandstoneSlab) Model() world.BlockModel {
...
func (s SmoothSandstoneSlab) UseOnBlock(pos world.BlockPos, face world.Face, clickPos mgl64.Vec3, w *world.World, user item.User, ctx *item.UseContext) (used bool) {
	clickedBlock := w.Block(pos)
	if clickedSlab, ok := clickedBlock.(SmoothSandstoneSlab); ok && !s.Double {
	...
	if sideSlab, ok := w.Block(pos.Side(face)).(SmoothSandstoneSlab); ok && !replaceableWith(w, pos, s) && !s.Double {
...
// BreakInfo ...
func (s SmoothSandstoneSlab) BreakInfo() BreakInfo {
	return BreakInfo{
		Hardness:    2,
...
func (s SmoothSandstoneSlab) LightDiffusionLevel() uint8 {
	if s.Double {
		return 15
	}
	return 0
...
// AABB ...
func (s SmoothSandstoneSlab) AABB(world.BlockPos, *world.World) []physics.AABB {
	if s.Double {
...
// EncodeItem ...
func (s SmoothSandstoneSlab) EncodeItem() (id int32, meta int16) {
	if s.Double {
		return 181, 6
	}
	return 182, 6
...
// EncodeBlock ...
func (s SmoothSandstoneSlab) EncodeBlock() (name string, properties map[string]interface{}) {
	if s.Double {
		return "minecraft:double_stone_slab2", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type_2": "smooth_sandstone"}
	}
	return "minecraft:stone_slab2", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type_2": "smooth_sandstone"}
}

// Hash ...
func (s SmoothSandstoneSlab) Hash() uint64 {
	return hashSmoothSandstoneSlab | (uint64(boolByte(s.Top)) << 32) | (uint64(boolByte(s.Double)) << 33)
}

// CanDisplace ...
func (s SmoothSandstoneSlab) CanDisplace(b world.Liquid) bool {
	_, ok := b.(Water)
	return !s.Double && ok
}

// SideClosed ...
func (s SmoothSandstoneSlab) SideClosed(pos, side world.BlockPos, _ *world.World) bool {
	// Only returns true if the side is below the slab and if the slab is not upside down.
	return !s.Top && side[1] == pos[1]-1
}

// allSmoothSandstoneSlabs returns all states of smooth sandstone slabs.
func allSmoothSandstoneSlabs() (slabs []world.Block) {
	f := func(double bool, upsideDown bool) {
		slabs = append(slabs, SmoothSandstoneSlab{Double: double, Top: upsideDown})
	}
	f(false, false)
	f(false, true)
	f(true, false)
