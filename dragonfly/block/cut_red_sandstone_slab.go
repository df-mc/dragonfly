...
// CutRedSandstoneSlab is a half block that allows entities to walk up blocks without jumping.
...
// Model ...
func (s CutRedSandstoneSlab) Model() world.BlockModel {
...
func (s CutRedSandstoneSlab) UseOnBlock(pos world.BlockPos, face world.Face, clickPos mgl64.Vec3, w *world.World, user item.User, ctx *item.UseContext) (used bool) {
	clickedBlock := w.Block(pos)
	if clickedSlab, ok := clickedBlock.(CutRedSandstoneSlab); ok && !s.Double {
	...
	if sideSlab, ok := w.Block(pos.Side(face)).(CutRedSandstoneSlab); ok && !replaceableWith(w, pos, s) && !s.Double {
...
// BreakInfo ...
func (s CutRedSandstoneSlab) BreakInfo() BreakInfo {
	return BreakInfo{
		Hardness:    2,
...
func (s CutRedSandstoneSlab) LightDiffusionLevel() uint8 {
	if s.Double {
		return 15
	}
	return 0
...
// AABB ...
func (s CutRedSandstoneSlab) AABB(world.BlockPos, *world.World) []physics.AABB {
	if s.Double {
...
// EncodeItem ...
func (s CutRedSandstoneSlab) EncodeItem() (id int32, meta int16) {
	if s.Double {
		return -168, 4
	}
	return -166, 4
...
// EncodeBlock ...
func (s CutRedSandstoneSlab) EncodeBlock() (name string, properties map[string]interface{}) {
	if s.Double {
		return "minecraft:double_stone_slab4", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type_4": "cut_red_sandstone"}
	}
	return "minecraft:stone_slab4", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type_4": "cut_red_sandstone"}
}

// Hash ...
func (s CutRedSandstoneSlab) Hash() uint64 {
	return hashCutRedSandstoneSlab | (uint64(boolByte(s.Top)) << 32) | (uint64(boolByte(s.Double)) << 33)
}

// CanDisplace ...
func (s CutRedSandstoneSlab) CanDisplace(b world.Liquid) bool {
	_, ok := b.(Water)
	return !s.Double && ok
}

// SideClosed ...
func (s CutRedSandstoneSlab) SideClosed(pos, side world.BlockPos, _ *world.World) bool {
	// Only returns true if the side is below the slab and if the slab is not upside down.
	return !s.Top && side[1] == pos[1]-1
}

// allCutRedSandstoneSlabs returns all states of cut red sandstone slabs.
func allCutRedSandstoneSlabs() (slabs []world.Block) {
	f := func(double bool, upsideDown bool) {
		slabs = append(slabs, CutRedSandstoneSlab{Double: double, Top: upsideDown})
	}
	f(false, false)
	f(false, true)
	f(true, false)
