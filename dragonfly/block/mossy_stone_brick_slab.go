...
// MossyStoneBrickSlab is a half block that allows entities to walk up blocks without jumping.
...
// Model ...
func (s MossyStoneBrickSlab) Model() world.BlockModel {
...
func (s MossyStoneBrickSlab) UseOnBlock(pos world.BlockPos, face world.Face, clickPos mgl64.Vec3, w *world.World, user item.User, ctx *item.UseContext) (used bool) {
	clickedBlock := w.Block(pos)
	if clickedSlab, ok := clickedBlock.(MossyStoneBrickSlab); ok && !s.Double {
	...
	if sideSlab, ok := w.Block(pos.Side(face)).(MossyStoneBrickSlab); ok && !replaceableWith(w, pos, s) && !s.Double {
...
// BreakInfo ...
func (s MossyStoneBrickSlab) BreakInfo() BreakInfo {
	return BreakInfo{
		Hardness:    1.5,
...
func (s MossyStoneBrickSlab) LightDiffusionLevel() uint8 {
	if s.Double {
		return 15
	}
	return 0
...
// AABB ...
func (s MossyStoneBrickSlab) AABB(world.BlockPos, *world.World) []physics.AABB {
	if s.Double {
...
// EncodeItem ...
func (s MossyStoneBrickSlab) EncodeItem() (id int32, meta int16) {
	if s.Double {
		return -168, 0
	}
	return -166, 0
...
// EncodeBlock ...
func (s MossyStoneBrickSlab) EncodeBlock() (name string, properties map[string]interface{}) {
	if s.Double {
		return "minecraft:double_stone_slab4", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type_4": "mossy_stone_brick"}
	}
	return "minecraft:stone_slab4", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type_4": "mossy_stone_brick"}
}

// Hash ...
func (s MossyStoneBrickSlab) Hash() uint64 {
	return hashMossyStoneBrickSlab | (uint64(boolByte(s.Top)) << 32) | (uint64(boolByte(s.Double)) << 33)
}

// CanDisplace ...
func (s MossyStoneBrickSlab) CanDisplace(b world.Liquid) bool {
	_, ok := b.(Water)
	return !s.Double && ok
}

// SideClosed ...
func (s MossyStoneBrickSlab) SideClosed(pos, side world.BlockPos, _ *world.World) bool {
	// Only returns true if the side is below the slab and if the slab is not upside down.
	return !s.Top && side[1] == pos[1]-1
}

// allMossyStoneBrickSlabs returns all states of mossy stone brick slabs.
func allMossyStoneBrickSlabs() (slabs []world.Block) {
	f := func(double bool, upsideDown bool) {
		slabs = append(slabs, MossyStoneBrickSlab{Double: double, Top: upsideDown})
	}
	f(false, false)
	f(false, true)
	f(true, false)
