...
// SmoothQuartzSlab is a half block that allows entities to walk up blocks without jumping.
...
// Model ...
func (s SmoothQuartzSlab) Model() world.BlockModel {
...
func (s SmoothQuartzSlab) UseOnBlock(pos world.BlockPos, face world.Face, clickPos mgl64.Vec3, w *world.World, user item.User, ctx *item.UseContext) (used bool) {
	clickedBlock := w.Block(pos)
	if clickedSlab, ok := clickedBlock.(SmoothQuartzSlab); ok && !s.Double {
	...
	if sideSlab, ok := w.Block(pos.Side(face)).(SmoothQuartzSlab); ok && !replaceableWith(w, pos, s) && !s.Double {
...
// BreakInfo ...
func (s SmoothQuartzSlab) BreakInfo() BreakInfo {
	return BreakInfo{
		Hardness:    2,
...
func (s SmoothQuartzSlab) LightDiffusionLevel() uint8 {
	if s.Double {
		return 15
	}
	return 0
...
// AABB ...
func (s SmoothQuartzSlab) AABB(world.BlockPos, *world.World) []physics.AABB {
	if s.Double {
...
// EncodeItem ...
func (s SmoothQuartzSlab) EncodeItem() (id int32, meta int16) {
	if s.Double {
		return -168, 1
	}
	return -166, 1
...
// EncodeBlock ...
func (s SmoothQuartzSlab) EncodeBlock() (name string, properties map[string]interface{}) {
	if s.Double {
		return "minecraft:double_stone_slab4", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type_4": "smooth_quartz"}
	}
	return "minecraft:stone_slab4", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type_4": "smooth_quartz"}
}

// Hash ...
func (s SmoothQuartzSlab) Hash() uint64 {
	return hashSmoothQuartzSlab | (uint64(boolByte(s.Top)) << 32) | (uint64(boolByte(s.Double)) << 33)
}

// CanDisplace ...
func (s SmoothQuartzSlab) CanDisplace(b world.Liquid) bool {
	_, ok := b.(Water)
	return !s.Double && ok
}

// SideClosed ...
func (s SmoothQuartzSlab) SideClosed(pos, side world.BlockPos, _ *world.World) bool {
	// Only returns true if the side is below the slab and if the slab is not upside down.
	return !s.Top && side[1] == pos[1]-1
}

// allSmoothQuartzSlabs returns all states of smooth quartz slabs.
func allSmoothQuartzSlabs() (slabs []world.Block) {
	f := func(double bool, upsideDown bool) {
		slabs = append(slabs, SmoothQuartzSlab{Double: double, Top: upsideDown})
	}
	f(false, false)
	f(false, true)
	f(true, false)
