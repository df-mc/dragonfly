...
type StoneSlab struct {
	...
	// StoneSlab is the type of stone of the slabs. This field must have one of the values found in the material
	// package.
	StoneSlab stone_slabs.StoneSlab
...
func (s StoneSlab) UseOnBlock(pos world.BlockPos, face world.Face, clickPos mgl64.Vec3, w *world.World, user item.User, ctx *item.UseContext) (used bool) {
	...
	if clickedSlab, ok := clickedBlock.(StoneSlab); ok && !s.Double {
		if (face == world.FaceUp && !clickedSlab.Double && clickedSlab.StoneSlab == s.StoneSlab && !clickedSlab.Top) ||
			(face == world.FaceDown && !clickedSlab.Double && clickedSlab.StoneSlab == s.StoneSlab && clickedSlab.Top) {
	...
	if sideSlab, ok := w.Block(pos.Side(face)).(StoneSlab); ok && !replaceableWith(w, pos, s) && !s.Double {
		...
		if !sideSlab.Double && sideSlab.StoneSlab == s.StoneSlab {
...
func (s StoneSlab) EncodeItem() (id int32, meta int16) {
	switch s.StoneSlab {
	case stone_slabs.SmoothStone():
		if s.Double {
			return 43, 0
		}
		return 44, 0
	case stone_slabs.Sandstone():
		if s.Double {
			return 43, 1
		}
		return 44, 1
	case stone_slabs.Wood():
		if s.Double {
			return 43, 2
		}
		return 44, 2
	case stone_slabs.Cobblestone():
		if s.Double {
			return 43, 3
		}
		return 44, 3
	case stone_slabs.Bricks():
		if s.Double {
			return 43, 4
		}
		return 44, 4
	case stone_slabs.StoneBrick():
		if s.Double {
			return 43, 5
		}
		return 44, 5
	case stone_slabs.Quartz():
		if s.Double {
			return 43, 6
		}
		return 44, 6
	case stone_slabs.NetherBrick():
		if s.Double {
			return 43, 7
		}
		return 44, 7
...
func (s StoneSlab) EncodeBlock() (name string, properties map[string]interface{}) {
	if s.Double {
		return "minecraft:double_stone_slab", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type": s.StoneSlab.String()}
	}
	return "minecraft:stone_slab", map[string]interface{}{"top_slot_bit": s.Top, "stone_slab_type": s.StoneSlab.String()}
}

// Hash ...
func (s StoneSlab) Hash() uint64 {
	return hashStoneSlab | (uint64(boolByte(s.Top)) << 32) | (uint64(boolByte(s.Double)) << 33) | (uint64(s.StoneSlab.Uint8()) << 34)
}
...
// allStoneSlabs returns all states of wood slabs.
func allStoneSlabs() (slabs []world.Block) {
	f := func(double bool, upsideDown bool) {
		slabs = append(slabs, StoneSlab{Double: double, Top: upsideDown, StoneSlab: stone_slabs.SmoothStone()})
		slabs = append(slabs, StoneSlab{Double: double, Top: upsideDown, StoneSlab: stone_slabs.Sandstone()})
		slabs = append(slabs, StoneSlab{Double: double, Top: upsideDown, StoneSlab: stone_slabs.Wood()})
		slabs = append(slabs, StoneSlab{Double: double, Top: upsideDown, StoneSlab: stone_slabs.Cobblestone()})
		slabs = append(slabs, StoneSlab{Double: double, Top: upsideDown, StoneSlab: stone_slabs.Bricks()})
		slabs = append(slabs, StoneSlab{Double: double, Top: upsideDown, StoneSlab: stone_slabs.StoneBrick()})
		slabs = append(slabs, StoneSlab{Double: double, Top: upsideDown, StoneSlab: stone_slabs.Quartz()})
		slabs = append(slabs, StoneSlab{Double: double, Top: upsideDown, StoneSlab: stone_slabs.NetherBrick()})
	}
	f(false, false)
	f(false, true)
	f(true, false)
