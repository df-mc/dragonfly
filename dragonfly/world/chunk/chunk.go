...
const (
	maxSubChunkY     = cube.MaxY >> 4
	minSubChunkY     = cube.MinY >> 4
	maxSubChunkIndex = maxSubChunkY - minSubChunkY
	subChunkCount    = maxSubChunkY - minSubChunkY + 1
)
...
func (chunk *Chunk) Light(x uint8, y int16, z uint8) uint8 {
	...
	return chunk.sub[i].Light(x&15, uint8(y&15), z&15)
}
...
func (chunk *Chunk) SkyLight(x uint8, y int16, z uint8) uint8 {
	...
	return chunk.sub[i].SkyLightAt(x&15, uint8(y&15), z&15)
}
...
func (chunk *Chunk) HighestLightBlocker(x, z uint8) int16 {
	for sy := maxSubChunkIndex; sy >= 0; sy-- {
		sub := chunk.sub[sy]
		...
		for y := 15; y >= 0; y-- {
			totalY := int16(y | (sy << 4))
			if FilteringBlocks[sub.storages[0].RuntimeID(x, uint8(y), z)] == 15 {
				return totalY
			}
		}
	}
...
func (chunk *Chunk) HighestBlock(x, z uint8) int16 {
	for sy := maxSubChunkIndex; sy >= 0; sy-- {
		...
		for y := 15; y >= 0; y-- {
			totalY := int16(y | (sy << 4))
			if sub.storages[0].RuntimeID(x, uint8(y), z) != chunk.air {
				return totalY
			}
		}
	}
...
func subY(y int16) uint8 {
	return uint8(y>>4) - minSubChunkY
}
